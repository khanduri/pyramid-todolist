...
def applcation_created_subscriber(event):
    log.warn('Initialize database ...')
...
def cose_view(request):
    tid = int(request.matchdict['id'])
    request.db.execute('update tasks set closed = ? where id = ?', (1, tid))
    request.db.commit()
